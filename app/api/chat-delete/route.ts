...
export async function POST(request: NextRequest) {
  console.log('=== DELETE CHAT API ROUTE START ===')
  ...
  try {
    ...
    console.log('🗑️ Session ID to delete:', sessionId)

    console.log('🗑️ Deleting chat session from n8n webhook')
    console.log('🔗 Webhook URL:', API_ENDPOINTS.N8N_WEBHOOKS.DELETE_CHAT)
    console.log('🔗 Full webhook URL:', API_ENDPOINTS.N8N_WEBHOOKS.DELETE_CHAT)
    ...
    const webhookUrl = API_ENDPOINTS.N8N_WEBHOOKS.DELETE_CHAT
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to delete chat: ${response.status} ${response.statusText}`, details: errorText },
    ...
    try {
      ...
      if (!responseText || responseText.trim() === '') {
        ...
        result = { success: true, message: 'Chat deleted successfully' }
      } else {
    ...
    console.log('=== DELETE CHAT API ROUTE END ===')
    return NextResponse.json(result)
  } catch (error) {
    console.error('❌ Error in chat delete proxy:', error)
    return NextResponse.json(
